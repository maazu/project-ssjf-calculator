...
CAL_OPERATION = 'exponent'
DEFAULT_RESULT_SAVE_PATH = '../results/' + CAL_OPERATION + \
    '/' + current_timestamp() + '-' + CAL_OPERATION + '.txt'


def perform_operation(number_one):
    result = Decimal(number_one).exp()
    return result


def exponent(number_one, number_two, save_file, show_output, path=DEFAULT_RESULT_SAVE_PATH,):
...
if __name__ == "__main__":
    ...
    exponent(number_one, number_two, save_file, show_output)
